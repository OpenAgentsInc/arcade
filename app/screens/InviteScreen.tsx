...
interface InviteScreenProps extends NativeStackScreenProps<AppStackScreenProps<"Invite">> {}

export const InviteScreen: FC<InviteScreenProps> = observer(function InviteScreen({
  route,
}: {
  route: any
}) {
  ...
  const inviteByPubkey = async (data) => {
    ...
    try {
      ...
      alert(`Invalid user, please make sure you enter valid public key or npub`)
    }
  }
        ...
        <BottomSheetView style={$modalContent}>
          ...
          <View style={$modalForm}>
            <Formik
              ...
              onSubmit={(values) => inviteByPubkey(values)}
            >
